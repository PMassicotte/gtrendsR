...
gconnect <- function(usr = NULL, psw = NULL, verbose = FALSE){
  ...
  if (is.null(usr)) {
    if (Sys.getenv("GOOGLE_USER") != "")
      usr <- Sys.getenv("GOOGLE_USER")
    if (getOption("google.user") != "")
      usr <- getOption("google.user")
    if (is.null(usr))
      stop("No Google Username / account supplied.", call. = FALSE)
  }
  if (is.null(psw)) {
    if (Sys.getenv("GOOGLE_PASSWORD") != "")
      psw <- Sys.getenv("GOOGLE_PASSWORD")
    if (getOption("google.password") != "")
      psw <- getOption("google.password")
    if (is.null(psw))
      stop("No Google password supplied.", call. = FALSE)
  }
...
.getGALX <- function(curl) {
  ...
  curlPerform(
    url = "https://accounts.google.com/accounts/ServiceLogin",
    curl = curl,
...
gtrends.default <-
  function(ch,
    ...
    countries[which(countries[, "COUNTRY"] == "Namibia"), "CODE"] <-
      "NA"
