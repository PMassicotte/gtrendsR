...
reshape_interest_over_time <- function(df, comparison_item, widget, tz) {
  ...
  df_long$geo <- ifelse(df_long$geo == "", "world", df_long$geo)
  df_long$gprop <- ifelse(
    widget$request$requestOptions$property[1] == "",
    "web",
    widget$request$requestOptions$property[1]
  )
  df_long$category <- widget$request$requestOptions$category[1]

  # Clean up column names and remove unnecessary columns
  names(df_long)[1] <- "date"
  df_long$id <- NULL
...
process_multirange_data <- function(df, comparison_item, widget, tz) {
  ...
  kw <- widget$request$comparisonItem[[2]]$complexKeywordsRestriction[[1]][[
    1
  ]]$value
  kw <- gsub("[[:blank:]-]", ".", kw)
  ...
  for (jj in seq_len(ncol(date_cols))) {
    ...
    # Add metadata
    meta_data <- comparison_item[rep(jj, n), 1:3, drop = FALSE]
    meta_data[, 1] <- ifelse(meta_data[, 1] == "", "world", meta_data[, 1])
    meta_data$gprop <- ifelse(
      widget$request$requestOptions$property[1] == "",
      "web",
      widget$request$requestOptions$property[1]
    )
    meta_data$category <- widget$request$requestOptions$category[1]
...
process_category_data <- function(df, comparison_item, widget, tz) {
  ...
  result_df <- cbind(df, comparison_item[rep(1, n), 2:5])
...
reshape_geographic_data <- function(df, widget, resolution, i) {
  ...
  df_long <- cbind(
    df_long,
    keywords[rep(seq_len(nrow(keywords)), each = n), 2, drop = FALSE],
    row.names = NULL,
...
extract_widget_keywords <- function(widget, i) {
  ...
  if (length(restriction$operator) == 0) {
    # Simple keyword list
    kw <- do.call(rbind, restriction$keyword)
  } else {
    # Complex keyword with operators
    value <- paste(restriction$keyword[[1]]$value, collapse = " + ")
    type <- unique(restriction$keyword[[1]]$type)
...
#'
#' @param region_results List of data frames with different geographic resolutions
